...
func BenchmarkPreallocLine(b *testing.B) {
...
func BenchmarkPreallocLine2(b *testing.B) {
	logger := iqlog.NewIQLogger(true)
	logger.SetDebugMode(false)
	logger.SetWriter(io.Discard)
	for i := 0; i < b.N; i++ {
		logger.WithPreallocLine2Info().Str("rate", "15").Int("low", 16).Float32("high", 123.2).Msg(msg)
		// logger.WithPreallocLineInfo()
	}
}

func BenchmarkBytesliceLine(b *testing.B) {
	logger := iqlog.NewIQLogger(true)
	logger.SetDebugMode(false)
	logger.SetWriter(io.Discard)
	for i := 0; i < b.N; i++ {
...
func BenchmarkBufferLine(b *testing.B) {
	logger := iqlog.NewIQLogger(true)
	logger.SetDebugMode(false)
	logger.SetWriter(io.Discard)
	for i := 0; i < b.N; i++ {
		// logger.Str("rate", "15").Int("low", 16).Float32("high", 123.2).Info(msg)
		logger.WithBufferLineInfo().Str("rate", "15").Int("low", 16).Float32("high", 123.2).Msg(msg)
		// logger.Info(msg)
	}
}

func BenchmarkBufferLineNL(b *testing.B) {
	logger := iqlog.NewIQLogger(true)
	logger.SetDebugMode(false)
	logger.SetWriter(io.Discard)
	for i := 0; i < b.N; i++ {
