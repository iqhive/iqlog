...
func TestTimestampLayouts(t *testing.T) {
	for _, layout := range []string{"seconds", "milliseconds", "microseconds", "nanoseconds", time.RFC822} {
		t.Run(layout, func(t *testing.T) {
			sb := &syncBuffer{}
			l := MustNew(Config{Format: FormatJSON, Writer: sb, IncludeTime: true, TimestampLayout: layout})
			l.Info("message")
			var record map[string]any
			if err := json.Unmarshal([]byte(sb.String()), &record); err != nil {
				t.Fatal(err)
...
func TestInjectedClockAndReservedFields(t *testing.T) {
	...
	if record["time"] != now.Format("2006-01-02T15:04:05.000000") {
